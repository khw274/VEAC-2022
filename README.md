...
차량의 더 원활한 주행을 위해 Pure Purshit 알고리즘의 Look Ahead Distance 파라미터 값을 수정했다.  
전방 주시 거리(Look Ahead Distance)를 길게 설정하면 부드러운 주행이 가능하지만 커브 구간에서 곡률이 증가하고 경로를 정확하게 따라가지 않을 수 있다.  
...
def get_str_angle(self, steer_p, wheel_base=0.39):
    # 조향점(steer_p)의 x, y 좌표를 각각 steer_y, steer_x에 저장,
    # 차량 좌표계(Car Coordinate System)는 차량의 진행 방향을 x축으로, 좌우 방향을 y축으로 정의한다. 따라서 좌표를 교차해서 저장한다.
    steer_x = steer_p[1]  # 조향점의 y 좌표 (차량 좌표계에서 전방 방향)
    steer_y = steer_p[0]  # 조향점의 x 좌표 (차량 좌표계에서 좌우 방향)
...
조향각을 구하는 함수를 간략히 설명하자면 우선 오프셋시켜 놓았던 우측으로 가장 가까운 라바콘 좌표를 불러와 교차해서 steer_x에 좌우 좌표, steer_y에 전방 좌표를 저장한다.
...
# - (steer_x / 2): 차량이 회전할 때, 조향점의 전방 거리 steer_x의 절반을 고려하여 회전 반경의 첫 번째 부분을 계산한다.
# - (steer_y ** 2) / (2 * steer_x); 조향점의 좌우 거리 steer_y의 제곱을 전방 거리 steer_x로 나누어, 조향점이 차량의 중심선에서  
  벗어나는 정도를 고려한 회전 반경의 두 번째 부분을 계산한다.
```
