...
def get_str_angle(self, steer_p, wheel_base=0.39):
    # 조향점(steer_p)의 x, y 좌표를 각각 steer_y, steer_x에 저장, 수학적 처리를 단순화하고, 차량의 회전 궤적을 정확히 계산하기 위해 교차해서 저장
...
조향각을 구하는 함수를 간략히 설명하자면 우선 오프셋시켜 놓았던 우측으로 가장 가까운 라바콘 좌표를 불러와 수학적 처리를 단순화하고, 차량의 회전 궤적을 정확히 계산하기 위해 교차해서 steer_x에 좌우 좌표, steer_y에 전방 좌표를 저장한다.
