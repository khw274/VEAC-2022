...
## 최종
해당 대회를 준비하면서 가장 중요하다고 느낀 부분은 수많은 다양한 시나리오 테스트를 통한 파라미터 값 조절이라고 생각한다.  

대회에서 장애물이 배치된 간격을 준비 과정에서는 알 수 없기 때문에 최대한 많은 시나리오를 설정해 테스트하고 그에 맞는 최선의 파라미터 값 설정이 수상 여부를 결정하는 것 같다.
