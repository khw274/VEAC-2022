...
'''dynamic = 움직임 o, high width
   static = 움직임 x, low width
   cone = 움직임 x, high width'''
...
class DynamicStop:
    ...
    def map_callback(self, msg): # callback lidar map coordinate
        pcd_list = []
        for i in msg.points:
            pcd = []
            pcd.append(i.x)
        ...
        '''알고리즘 부분'''
        if self.is_status and self.is_local_path: # 다루는 데이터가 callback이 다 된 상태에서 알고리즘 실행(global_path는 일정 주기마다 callback하기 때문에 제외)
            if self.cone_mission is False: # 콘 미션 제어값과 겹치지 않기 위한 조건문
                # local parameter initalize
                stop_distance = 1.35
                dynamic_vel = 999
                selected_lane = 1
                if self.static_mission is False: # 주행 차선에 장애물이 있다면 장애물 위치를 특정 짓기 위한 조건문
                    local_obstacle_list = []
                    '''local path 에 장애물이 존재하는지 검사 
                    local path 길이는 속도가 클수록 늘어남'''
                    for i in pcd_list:
                        for j in self.local_path_list:
                            dst = self.get_dst(i, j)
                            if dst < 0.15: # 차선 폭은 대략 0.36m으로 주행 차선 중앙 기준 양쪽 0.15m 장애물 여부 확인
                                local_obstacle_list.append(i)
                                print('전방에 장애물')
                            elif dst < 1.25: # 주행 차선 근처에 장애물이 감지되면 서행 -> 동적 장애물이 주행 차로에 다가올때를 대비
                            # elif dst < 1.23: # 주행 차선 근처에 장애물이 감지되면 서행 -> 동적 장애물이 주행 차로에 다가올때를 대비
                                dynamic_vel = 2.0
                                print('근처에 장애물')
                    ...
                    # 10동안 장애물이 주행 차선에 머물기 때문에 정적 장애물
                    if self.tic_tok >= 30: 
    ...
    def callback(self, msg): # callback lidar coordintae data
        lfd = 0.5
        track_width = 1.2
        ...
        if self.is_status and self.is_local_path: # 다루는 데이터가 callback이 다 된 상태에서 알고리즘 실행(global_path는 일정 주기마다 callback하기 때문에 제외)
            ...
            left_objs = []
            right_objs = []
            for i in around_obj_list: # lidar 좌표계 y축을 기준으로 좌우 구분
                if i[1] > 0:
                    left_objs.append(i)
                else:
                    right_objs.append(i)
            ...
            if self.cone_mission:
                ...
                for cone in right_objs:
                    ...
                    if min_dst> dst - lfd and dst - lfd > 0:
                        min_dst=dst
                        cur_cone = cone # lfd 만큼 앞에 있는 오른쪽 콘 중 가자 가까운 콘 검출
    ...
    def get_str_angle(self, steer_p, wheel_base=0.39): # 조향각 계산하는 함수
        steer_x = steer_p[1]
        steer_y = steer_p[0]
